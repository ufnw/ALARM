...
def set_time(): # Создаём новую функцию, для установки времени и учёта предпочтений пользователя
    ...
    try:
        ...
        time = input("\nВведите время для будильника в формате ЧЧ:ММ: ")
        ...
        message = input("\nВведите сообщение, которое нужно отправить при срабатывании будильника. Если сообщение отсутствует, укажите 0: ")
        number = int(input(f"\nВведите номер звука {sounds}: "))
    ...
    try:
        while True:
            if str(time) == str(datetime.datetime.now().strftime("%H:%M")):
                if int(message) == 0:
                    print("\n Пора вставать!" * 10)                          # Печатаем сообщение при срабатывании будильника
...
def functional(): # Функция, которая будет содержать в себе остальные функции
    try:
        print("\n Загрузка..." * 4) # 10 раз пишем слово "Загрузка...", чтобы не было видно сообщение от PyGame
